...
func manageServer() {
	...
	http.HandleFunc("/json/eliminarImagen", func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		fmt.Fprintln(w, "Se ha eliminado las imagen correctamente")
	})
	...
	http.HandleFunc("/json/gestionarImagenesVM", func(w http.ResponseWriter, r *http.Request) {
		// Verificar que el método HTTP sea POST
		if r.Method != http.MethodPost {
			http.Error(w, "Método no permitido", http.StatusMethodNotAllowed)
			return
		...
		w.WriteHeader(http.StatusOK)
		fmt.Fprintln(w, "Se han revisado las imagenes correctamente")
	...
	http.HandleFunc("/json/gestionContenedor", func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		fmt.Fprintln(w, "Se a borrado el contenedor correctamente")
	...
	for {
		...
		if currentQueueSize > 0 {
			...
			case "revisar":
				...
				go RevisarContenedores(ip, hostname)
...
func RevisarImagenes(ip, hostname string) ([]Imagen, error) {
	...
	for i := 0; i < len(res); i++ {
		if tabla == 4 {
			datos[tabla] = res[i]
			imagenes = append(imagenes, ingresarDatosImagen(datos))
			tabla = 0
			datos = make([]string, 5)
		} else {
			datos[tabla] = res[i]
...
func ingresarDatosImagen(datos []string) Imagen {
...
func RevisarContenedores(ip, hostname string) ([]Conetendor, error) {
	...
	for i := 0; i < len(res); i++ {
		if tabla == 6 {
			datos[tabla] = res[i]
			contenedores = append(contenedores, ingresarDatosContenedor(datos))
			tabla = 0
			conetendor++
			datos = make([]string, 7)
		} else {
...
func ingresarDatosContenedor(datos []string) Conetendor {
...
func decodeBase64(base64String string) ([]byte, error) {
	return ioutil.ReadAll(base64.NewDecoder(base64.StdEncoding, strings.NewReader(base64String)))
}
